...
const FROM_EMAIL = "InnovMate <noreply@konisberg.studio>";
...
export async function sendProjectInvitationEmail(
  ...
  try {
    const { data, error } = await resend.emails.send({
      ...
      to: [email],
      subject,
      text: `${inviterName} invited you to join "${projectName}" on InnovMate. Accept here: ${invitationUrl}`,
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
          <h2>Project Invitation</h2>
          <p><strong>${inviterName}</strong> has invited you to join the project <strong>"${projectName}"</strong> on InnovMate.</p>
...
export async function sendSubsEmail(email: string): Promise<void> {
  const InnovMateUrl = `${BASE_URL}`;
  const subject = `Subscription to InnovMate`;
  ...
  try {
    const { data, error } = await resend.emails.send({
      ...
      to: [email],
      subject,
      text: `Thank you for subscribing to InnovMate! You can start using it here: ${InnovMateUrl}`,
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
          <h2>Subscription Confirmation</h2>
          <p>Thank you for subscribing to InnovMate! We're excited to have you on board.</p>
          <p>You can start using InnovMate by clicking the button below:</p>
          <a href="${InnovMateUrl}" style="display:inline-block;padding:10px 20px;background:#4f46e5;color:white;text-decoration:none;border-radius:5px;">InnovMate Home</a>
          <p>If the button doesn't work, copy and paste this link: ${InnovMateUrl}</p>
        </div>
      `,
    });
